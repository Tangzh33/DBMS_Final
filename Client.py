...
class ChatClient:
    ...
    def create_group(self,groupname):
        ...
        self.send_string_with_length(groupname)
        print('success')

    def join_group(self,user,groupid):
        self.sk.sendall(bytes("6", "utf-8"))   
        print("##############",user,groupid)
        self.send_string_with_length(user)
        self.send_string_with_length(groupid)

    def exit_group(self,user,groupid):
        self.sk.sendall(bytes("7", "utf-8"))   
        print("##############",user,groupid)
        self.send_string_with_length(user)
        self.send_string_with_length(groupid)
    
    def get_all_groupinfo(self,user,groupid):
        self.sk.sendall(bytes("8", "utf-8"))   
        print("##############",user)
        self.send_string_with_length(user)
    
    def alter_account(self,uid, name, nickname, gender, hobby, password):
        self.sk.sendall(bytes('e', "utf-8"))   
        print(uid, name, nickname, gender, hobby, password)
        self.send_string_with_length(uid)
        self.send_string_with_length(name)
        self.send_string_with_length(nickname)
        self.send_string_with_length(gender)
        self.send_string_with_length(hobby)
        self.send_string_with_length(password)

    def get_all_history(self,uid):
        self.sk.sendall(bytes("15", "utf-8"))   
        print("##############",uid)
        self.send_string_with_length(uid)
    ########################### 封装一些发送接受数据的方法 ##############################
    # 发送带长度的字符串
