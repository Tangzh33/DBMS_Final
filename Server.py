...
def handle_create_group(_conn, addr):
    print('handle_create_group执行了!!')
    content = recv_all_string(_conn)
    print(content)
    # mid_msg = conn2user[_conn]
    time_msg = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    groupname_msg = content
    print( time_msg, groupname_msg)
    # 存入数据库
    db.create_group( time_msg, groupname_msg)
    return True

def handle_join_group(_conn, addr):#返回是否执行成功
    print('handle_join_group执行了!!')
    user = recv_all_string(_conn)
    groupid = recv_all_string(_conn)
    print(user ,groupid)
    # 存入数据库
    
    return db.join_group( user, groupid)

def handle_exit_group(_conn, addr):#返回是否执行成功
    print('handle_exit_group执行了!!')
    user = recv_all_string(_conn)
    groupid = recv_all_string(_conn)
    print(user ,groupid)
    # 存入数据库
    return db.exit_group_database( user, groupid)
    

def handle_alter_account(_conn, addr):
    print('handle_alter_account执行了!!')
    uid= recv_all_string(_conn)
    name= recv_all_string(_conn)
    nickname= recv_all_string(_conn)
    gender= recv_all_string(_conn)
    hobby= recv_all_string(_conn)
    password= recv_all_string(_conn)
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!神秘print 别删了
    print("[Log] Server uid, name, nickname, gender, hobby, password",uid, name, nickname, gender, hobby, password)
    # 存入数据库
    if db.alter_account( uid, name, password, nickname, gender, hobby):
        return True
    return False
...
def handle(_conn, addr):
    try:
        while True:
            ...
            elif _type == "5":  # 创建group
                print("开始处理创建群组请求")
                _goon = handle_create_group(_conn, addr)
            elif _type == "6":  # 加入group
                print("开始处理加入群组请求")
                _goon = handle_join_group(_conn, addr)
            elif _type == "7":  # 退出group
                print("开始处理退出群组请求")
                _goon = handle_exit_group(_conn, addr)
            elif _type == "e":  # 修改信息
                print("开始处理修改信息請求")
                _goon = handle_alter_account(_conn, addr)
